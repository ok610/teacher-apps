...
const Timer = (() => {
  ...
  function cacheEls(root) {
    ...
    els.presetLabel = document.getElementById("timer-preset-label");
  ...
  function handleFavoriteClick(event) {
    ...
    els.presetLabel.innerHTML = fav.name ? `설정 : <strong>${escapeHtml(fav.name)}</strong>` : "";
    setPhase("idle");
    els.stateText.textContent = "준비";
    renderRing();
    updateTimeDisplay();
